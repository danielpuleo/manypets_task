...
def elbow_method(X):
    ...
    # Fit KMeans for different numbers of clusters (e.g., from 1 to 10)
    for i in range(1, 11):
        kmeans = KMeans(n_clusters=i, random_state=42)
        kmeans.fit(X)
        wcss.append(kmeans.inertia_)  # inertia_ gives WCSS for the model
...
def diff_calculation(a,b,niter=5000,sampleSize=1000):
    diffMeans = []
...
def plot_dataframe_summary(df):

    # Creates barplot for categorical variables
    # And histogram for numeric variables
